# script2_between_participant_level_pca_scatter.py
...
# Compute average PCA components per participant and condition
df_avg = df_pca.groupby(['Participant', 'Condition']).mean().reset_index()

# Plotting
plt.figure(figsize=(10, 8))
conditions_unique = df_avg['Condition'].unique()
colors = {'D': 'blue', 'ND': 'red'}

for condition in conditions_unique:
    indices = df_avg['Condition'] == condition
    plt.scatter(
        df_avg.loc[indices, 'PCA1'],
        df_avg.loc[indices, 'PCA2'],
...
for idx, row in df_avg.iterrows():
    plt.text(row['PCA1'] + 0.02, row['PCA2'] + 0.02, str(int(row['Participant'])), fontsize=9)

plt.title('PCA Scatter Plot Between Participants (Average PCA per Participant)')
plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.legend(title='Condition')
plt.grid(True)
plt.tight_layout()
plt.savefig('pca_scatter_between_participants.png', dpi=300)
plt.show()
